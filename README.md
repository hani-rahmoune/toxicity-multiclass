...
- Approximately 6,000 molecules after cleaning  
...
XGBoost achieves the highest decision-level performance (**macro F1**), while ChemBERTa provides superior ranking ability (**ROC-AUC**).  

These results highlight the trade-off between ranking and classification performance in imbalanced multi-label settings.
...
1. **Data cleaning**  
Run `01_exploring_cleaning_data.ipynb` (or `prepare_data.py`) to process and standardize raw SMILES.

2. **Featurization**  
Execute `make_fingerprints.py` to generate 2048-bit Morgan fingerprints.

3. **Model training**  
    - **XGBoost:** `02_xgboost_baseline.ipynb`  
    - **MLP:** `03_mlp_baseline.ipynb`  
    - **ChemBERTa:** `05_chemberta_training.ipynb`
